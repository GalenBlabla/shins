...
class OrderModel(models.Model):
    ...
    out_trade_no = fields.CharField(
        max_length=50, unique=True, description="订单号，唯一标识每一笔订单"
    )
    total_amount = fields.DecimalField(
        max_digits=10, decimal_places=2, description="订单总金额"
    )
    ...
    status = fields.CharField(
        max_length=20,
        default="PENDING",
        description="订单状态，如待支付（PENDING）、已支付（PAID）、已取消（CANCELED）等",
    )
