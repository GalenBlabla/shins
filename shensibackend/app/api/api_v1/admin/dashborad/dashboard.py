...
async def daily_quota_usage():
    ...
    daily_usage = await Logs.annotate(
        date=RawSQL("DATE(FROM_UNIXTIME(`created_at`))"),
...
async def usage_habits():
    ...
    try:
        ...
        usage_habits_list = await Logs.annotate(
            date=RawSQL("DATE(FROM_UNIXTIME(`created_at`))"),
            count=Count('id')
        ).filter(
            date=today
        ).group_by('type').order_by('-count').values_list('type', 'count')
...
async def retention_rate():
    ...
    # 获取昨天的用户ID列表
    user_ids_yesterday = await Logs.annotate(
        date=RawSQL("DATE(FROM_UNIXTIME(`created_at`))")
    ).filter(
        date=yesterday
    ).values_list('user_id', flat=True)
    ...
    # 获取今天的用户ID列表
    user_ids_today = await Logs.annotate(
        date=RawSQL("DATE(FROM_UNIXTIME(`created_at`))")
    ).filter(
        date=today
    ).values_list('user_id', flat=True)
    ...
    # 计算留存的用户数
    retained_users = distinct_user_ids_today.intersection(
        distinct_user_ids_yesterday)
    retained_users_count = len(retained_users)

    new_users_count = len(distinct_user_ids_yesterday)
    retention = (retained_users_count / new_users_count) * \
        100 if new_users_count > 0 else 0
